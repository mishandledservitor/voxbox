#!/usr/bin/env python3
"""
Shortlisted STT — ElevenLabs Scribe wrapper exposing the full API surface
for client-call transcription.

Designed to be driven by shortlisted_gui.py but also usable from the CLI.
"""
...
def process_file(client, audio_path, *, out_format, output_path, speakers, language,
    ...
    print(f"🧠 Model: {model_id_label(speakers)}  (id={model_id_from_speakers()})")
    ...
    try:
        response = transcribe(
            client, audio_path,
            model_id=model_id_from_speakers(),
            speakers=speakers,
            language=language,
...
# Model id is fixed in current Scribe API; keep helpers in case ElevenLabs
# adds the experimental variant later. shortlisted_gui.py overrides via env.
def model_id_from_speakers():
    return os.environ.get("SHORTLISTED_MODEL_ID", "scribe_v1")


def model_id_label(_speakers):
    mid = model_id_from_speakers()
...
def main():
    p = argparse.ArgumentParser(description="Shortlisted STT — ElevenLabs Scribe with full options")
    ...
    p.add_argument("--model", choices=["scribe_v1", "scribe_v2"],
                   default="scribe_v2")
    ...
    os.environ["SHORTLISTED_MODEL_ID"] = args.model
