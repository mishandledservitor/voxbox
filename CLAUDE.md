...
9. **True TDD — red, green, refactor.** No production code without a failing test first. For
   every unit of work: write a test that fails for the right reason (red), write the minimum code to make it pass (green), then clean up with the tests still green (refactor). This is not a separate phase bolted onto Spec Kit — it happens inside `/speckit-implement`: each task's test is written and shown failing before that task's implementation is written.
